...
def summarize_clusters(shop_ids, X, labels, probs, persistence=None, top_k=5):
    ...
    print("\n--- Per-cluster quality (sort by size, mean_prob) ---")
...
def analyze_outliers(shop_ids, labels, outlier_scores=None, top_k=20):
    ...
    if outlier_scores is None:
        print("No outlier_scores_.")
        print("Several outliers: shop_id:", shop_ids[out_idx[:min(10, len(out_idx))]])
        return None
    ...
    print(f"Top {len(out_df)} outliers by outlier_score:")
...
def soft_assign_outliers(Z, labels, probs=None, q=0.95, slack=1.2):
    """
    Soft-assign HDBSCAN outliers (-1) to the nearest cluster in the Z space.
    Threshold per cluster = q-percentile of member distances to the centroid * slack.
    Returns: final_labels, assign_confidence, nearest_cluster, nearest_dist, per_cluster_threshold
    """
...
def scenario_metrics(labels, probs, persistence):
    ...
    if persistence is not None and len(unique) > 0:
        # cluster_persistence_ is ordered by cluster labels (0..k-1) in the internal sense.
        # The safest approach is to take persistence according to the order of unique cluster labels
        # only if the labels are 0..k-1. If not, we will create a mapping as well.

        pers_map = {}
        for i, c in enumerate(sorted(unique)):
            # If clusters are numbered 0..k-1, then i == c. But this is not always the case.
            # cluster_persistence_ is indexed by the internal cluster ID,
            # which usually corresponds to the label. If that is not the case for you,
...
def plot_2d_and_save(Z2, labels, title, save_path, dpi=200):
    """
    Calls the existing plot_2d(...) function and then saves the current figure to save_path.
    """
    plot_2d(Z2, labels, title=title)  #
...
print("===================================================")
print(f"=== CLUSTER ANALYSIS PIPELINE STARTS  ===")
print("===================================================")

#PATHS
TRAIN_DATA = "data/extension2/shop_embeddings/train.csv"
...
print("===================================================")
print(f"=== CLUSTER ANALYSIS PIPELINE FINISHED  ===")
print("===================================================")
