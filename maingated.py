...
if __name__ == "__main__":
    ...
    SAVE_OPEN_GATED_COVARIATE = True  # system gating: forecast * Open (known future)
    ...
    for CONTEXT_LEN in CONTEXT_LENGTHS:
        ...
        for STORE_ID in tqdm(store_list, desc=f"CTX {CONTEXT_LEN} forecasts", unit="store"):
            ...
            # Optional system gating: force forecast to zero when Open==0 (known-future covariate)
            if SAVE_OPEN_GATED_COVARIATE and ("Open" in df_test.columns):
                gated_path = os.path.join(ctx_output_dir, f"covariate_gated_store_{STORE_ID}.csv")

                q = pd.read_csv(cov_path)
                gate_df = pd.DataFrame(
                    {
                        "timestamp": df_test["timestamp"].astype(str),
                        "gate": pd.to_numeric(df_test["Open"], errors="coerce").fillna(1).astype(float).values,
                    }
                )

                q["timestamp"] = q["timestamp"].astype(str)
                q = q.merge(gate_df, on="timestamp", how="left")

                for col in ["p10", "median", "p90", "0.1", "0.5", "0.9", "predictions"]:
                    if col in q.columns:
                        q[col] = pd.to_numeric(q[col], errors="coerce") * q["gate"]

                q = q.drop(columns=["gate"])
                q.to_csv(gated_path, index=False)
